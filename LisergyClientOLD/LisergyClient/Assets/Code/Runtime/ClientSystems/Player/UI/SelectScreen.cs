...
namespace Assets.Code.UI
{
    ...
    public class SelectScreen<T> : GameUi, IEventListener
    {
        ...
        private SelectScreenParam<T> _screenParams;
        ...
        public override void OnOpen()
        {
            ...
            _screenParams = GetParameter<SelectScreenParam<T>>();
            _preview = new PreviewContainer(Root);

            foreach (var i in _screenParams.Datasource)
            {
                var element = _screenParams.CreateSelection(i);
                _scroll.Add(element);
                _items.Add(element);
                element.OnClicked = () =>
                {
                    _screenParams.OnPreview.Invoke(i, _preview);
