        ...
        public async UniTask StartMovement(EntityMovementRequestStarted ev)
        {
            ...
            for (var x = 0; x < path.Count; x++)
            {
                ...
                if (hasNext)
                {
                    ...
                    await clientPath.AddPath(next.Entity.GetView<TileView>(), view.GameObject.Location.ToVector(), direction);
                }
                if (hasPrevious)
                {
                    var previous = path[x - 1];
                    var direction = nodeTile.GetDirection(previous);
                    await clientPath.AddPath(nodeTile.Entity.GetView<TileView>(), view.GameObject.Location.ToVector(), direction);
                }
            }
        }
