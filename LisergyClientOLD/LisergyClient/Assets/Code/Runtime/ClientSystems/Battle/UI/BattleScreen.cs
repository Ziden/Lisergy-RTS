    ...
    public class BattleScreen : GameUi, IEventListener
    {
       ...
       // public event Action OnFinishedPlayback;
        ...
        private void OnSceneLoaded()
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                if (i < setup.Attacker.Units.Length) AddHealthbar(Units[setup.Attacker.Units[i].UnitID]);
                if (i < setup.Defender.Units.Length) AddHealthbar(Units[setup.Defender.Units[i].UnitID]);
            }
        ...
        private async Task PrepareBattleSceneAsync()
        {
            ...
            await _battlePlayback.SetupScene(scenario.transform, () => OnSceneLoaded());
        }

        public override void OnOpen()
        {
            _uiService.Close<GameHudScreen>();
