...
{
	private readonly GameSpec _specs;
    ...
    public async Task SetupPlayfab()
	{
		...
		await PlayFabServerAPI.SetTitleDataAsync(new SetTitleDataRequest
		{
			Key = "GameData",
			Value = serialized
		});
		...
		var playfabTypes = new HashSet<int>(response.Result.VirtualCurrencies.Select(s => int.Parse(s.CurrencyCode)));
