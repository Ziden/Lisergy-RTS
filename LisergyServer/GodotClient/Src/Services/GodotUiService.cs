...
public class GodotUiService : IUiService
{
	...
	private readonly Dictionary<Type, GameUi> _loadedScreens = new();
	private readonly IGameLog _log;
	private readonly Dictionary<Type, GameUi> _openScreens = new();
	...
	public void Close<T>() where T : GameUi
	{
		...
		var screen = _openScreens[typeof(T)];
		screen.OnClose();
	...
	public void Destroy<T>() where T : GameUi
	{
		...
		if (_openScreens.ContainsKey(typeof(T))) _openScreens.Remove(typeof(T));
	}
