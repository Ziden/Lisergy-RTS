...
public static class ClientExtensions
{
	...
	/// </summary>
	public static bool IsMine(this GameId id)
	{
		return ClientServices.ServerSdk.Server.Player.PlayerId == id;
	}

	/// <summary>
	///     Checks if a given entity belongs to the local player
	/// </summary>
	public static bool IsMine(this IEntity e)
	{
		return e.OwnerID.IsMine();
	}

	/// <summary>
	///     Checks if a given tile is visible to the local player
	/// </summary>
	public static bool IsVisible(this IEntity tile)
	{
		return tile != null && tile.Logic.Vision.GetEntitiesViewing().Any(p => p.IsMine());
	}

	/// <summary>
	///     Gets the Godot position of a given entity
	/// </summary>
	public static Vector3 GodotPosition(this IEntity entity)
	{
		return new Vector3(entity.GetTile().X, 0, entity.GetTile().Y);
	}

	/// <summary>
	///     Gets the Godot position of a given tile
	/// </summary>
	public static Vector3 GodotPosition(this TileModel entity)
	{
		return new Vector3(entity.X, 0, entity.Y);
	}
	...
	public static T GetView<T>(this IEntity entity)
	{
		return (T) entity.GetView();
	}
	...
	{
		task.ContinueWith(task => { callback(task.Result); }, TaskContinuationOptions.ExecuteSynchronously);
	}
	...
	public static void HideTab(this TabContainer tabs, int tabIndex)
	{
		...
		var tabContent = tabs.GetTabControl(tabIndex);
		if (tabContent != null)
		{
	...
	{
		if (node is T tt) return tt;
		foreach (var child in node.GetChildren())
		{
			var result = child.FindFirstOfType<T>();
	...
	public static TimeBlockTask OnFinish(this TimeBlock time, Action onFinish)
	{
		return new TimeBlockTask(ClientServices.ServerSdk, time, onFinish);
	}
	#endregion
}