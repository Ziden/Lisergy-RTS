...
///     We also place the entity on the logic of the game so we can easily check which entity is in which tile and also
///     take
///     advantage of logical events for the line of sight (exploration)
/// </summary>
public class EntityPositionListener : IAutoRegisterListener
{
	...
	{
		if (e.Action != EntityAction.MOVE) return;
		_client.Server.Actions.MoveEntity(e.TargetEntity, e.TargetTile, CourseIntent.Defensive);
	}
	...
	private void InterpolationStart(MovementInterpolationStartEvent e)
	{
		...
		if (view is IAnimatedSpriteEntity anim) anim.UpdateAnimation(e.From.GetDirection(e.To), true);
	...
	public void OnComponentModified(IEntity entity, MapPlacementComponent oldComponent,
		MapPlacementComponent newComponent)
	{
	...
	private void UpdateEntityPosition(IEntity e, MapPlacementComponent oldC, MapPlacementComponent newC)
	{
		...
		else if (view.Entity.Components.Has<MapPlacementComponent>() && view.State == EntityViewState.RENDERED)
