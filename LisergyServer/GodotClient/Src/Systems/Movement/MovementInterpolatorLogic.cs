...
public class MovementInterpolatorLogic
{
	...
	private readonly Queue<(TileModel, TileModel)> _queue = new();
	...
	public void InterpolateMovement(TileModel from, TileModel to)
	{
		...
		var node3D = gameObject.Get<Node3D>();
		if (node3D == null)
		{
		...
		var tilePos = to.Position.ToGodotVector3();
		var finalPos = new Vector3(tilePos.X, node3D.GlobalPosition.Y, tilePos.Z);
	...
	private void OnFinish(TileModel from, TileModel to)
	{
		...
		if (_queue.TryDequeue(out var newMove)) InterpolateMovement(newMove.Item1, newMove.Item2);
	}
}