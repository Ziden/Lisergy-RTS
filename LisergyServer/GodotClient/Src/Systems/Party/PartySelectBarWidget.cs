...
public class PartySelectBarWidget : IEventListener
{
	...
	private readonly List<PartyButton> _buttons = new();
	...
	public PartySelectBarWidget(params Button[] buttons)
	{
		...
		_assets.LoadGetTexture(AssetConfigs.ICON_MISSING_UNIT).Then(asset => { _missingUnitIcon = asset; });
	...
	{
		foreach (var b in _buttons) b.Button.ButtonPressed = false;
	...
	private async Task SetIcon(IEntity party, PartyButton b)
	{
		...
		if (_state.SelectedParty.Value == party) b.Button.ButtonPressed = true;
	}
