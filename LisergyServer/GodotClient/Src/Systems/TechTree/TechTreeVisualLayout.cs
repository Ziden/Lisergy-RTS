...
public class TechTreeVisualLayout<T>
{
	...
	private async Task PopulateTechTree(NodeTree<T> root)
	{
		...
		var tempPositions = new Dictionary<T, Vector2>();
	...
	private void CalculateNodePositionsTemp(NodeTree<T> node, Vector2 position, Dictionary<T, Vector2> positions)
	{
		...
		if (childCount > 0)
		{
			...
			var startX = position.X - totalWidth / 2;

			var i = 0;
			foreach (var child in children)
			{
				var childX = startX + i * HorizontalSpacing;
				var childY = position.Y + VerticalSpacing;
	...
	private void CalculateNodePositions(NodeTree<T> node, Vector2 position)
	{
		...
		if (childCount > 0)
		{
			...
			foreach (var child in children)
			{
				...
				var childPosition = new Vector2(Mathf.Round(childX), Mathf.Round(childY));
	...
	private void DrawConnectionLines(NodeTree<T> node)
	{
		...
		{
			var startPos = _nodePositions[node.Data] + new Vector2(NodeSize / 2, NodeSize / 2);
			var endPos = _nodePositions[child.Data] + new Vector2(NodeSize / 2, NodeSize / 2);
			...
			var midY = startPos.Y + (endPos.Y - startPos.Y) / 2;
	...
	{
		var position = _nodePositions[node.Data];
		...
		foreach (var child in node.ChildrenNodes()) await CreateBuildingNodes(child);
	}
}