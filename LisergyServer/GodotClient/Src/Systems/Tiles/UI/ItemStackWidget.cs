...
public partial class ItemStackWidget : GameUiWidget
{
	...
	public void SetColor(Color c)
	{
		...
		{
			GD.PrintErr(GetArt() + " does not have a panel stylebox");
		}
	}
	...
	public void SetData(ArtSpec icon, string name, int amount)
	{
		Load();
		_name.Text = name;
		_amount.Text = amount < 0 ? "" : "x" + amount;
		ClientServices.Assets.LoadGetTexture(icon).Then(tex => { _icon.Texture = tex; });
	}
	...
	public void SetData(TileSpec tile)
	{
		...
		_amount.Text = "x1";
		ClientServices.Assets.LoadGetTexture(tile.Icon).Then(tex => { _icon.Texture = tex; });
	}
	...
	public void SetData(ResourceStackData resource)
	{
		...
		ClientServices.Assets.LoadGetTexture(spec.Art).Then(tex => { _icon.Texture = tex; });
	}

	public override ArtSpec GetArt()
	{
		return AssetConfigs.WIDGET_ITEM_STACK;
	}
}
