...
public class GodotObjectPool
{
	...
	private readonly List<Node> _inactive = new();
	...
	public void Release(Node obj)
	{
		if (_active.Remove(obj))
		{
			...
			if (obj is Node3D n) n.Visible = false;
		}
	}
	...
	public Node Obtain()
	{
		if (_inactive.Count > 0)
		{
			...
			if (pooled is Node3D n) n.Visible = false;
			return pooled;
		}
	...
	public void Clear()
	{
		foreach (var node in _inactive) node.QueueFree();
		_inactive.Clear();

		foreach (var node in _active) node.QueueFree();
		_active.Clear();
	}
}