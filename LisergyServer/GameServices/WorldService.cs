...
public class WorldService : IEventListener
{
    ...
    private void OnBattleResultPacket(BattleResultPacket packet)
	{
		...
		{
			_world.Game.Log.Error(
				$"Battle result received for non-existent entities: Attacker={packet.Header.Attacker.EntityId}, Defender={packet.Header.Defender.EntityId}");
			return;
		}
		...
		if (atkPlayer != null)
		{
			...
			if (defenderEntity != null && defenderEntity.Logic.BattleGroup != null &&
			    !defenderEntity.Logic.BattleGroup.IsDestroyed)
		...
		if (defPlayer != null && defenderEntity != null)
		{
			...
			if (attackerEntity != null && attackerEntity.Logic.BattleGroup != null &&
			    !attackerEntity.Logic.BattleGroup.IsDestroyed)
	...
	private void OnBattleTrigger(BattleTriggeredEvent ev)
	{
		...
		{
			_world.Game.Log.Error(
				$"Battle triggered for non-existent entities: Attacker={ev.Attacker.EntityId}, Defender={ev.Defender.EntityId}");
			return;
		}

		_world.Game.Network.SendToServer(new BattleQueuedPacket(ev.BattleID, attackerEntity, defenderEntity),
			ServerType.BATTLE);
	}
}