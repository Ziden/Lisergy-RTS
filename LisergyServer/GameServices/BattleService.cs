...
public class BattleTaskExecutor : ITaskExecutor
{
	...
	public BattleTaskExecutor(TurnBattle battle)
	{
		Battle = battle;
	}
...
public class BattleService : IEventListener
{
	...
	public Dictionary<GameId, GameTask> BattleTasks { get; } = new();
	public Dictionary<GameId, BattleLogPacket> AllBattles { get; } = new();
    ...
    public void OnBattleTrigger(BattleQueuedPacket packet)
	{
		...
		_game.Network.SendToPlayer(
			new BattleStartPacket(packet.BattleID, packet.Position, packet.Attacker, packet.Defender),
			GetAllPlayers(battle).ToArray());
	}
    ...
    private void OnBattleFinishedProcessing(BattleFinishedTaskEvent ev)
	{
		...
		_game.Network.SendToServer(fullResultPacket);
		BattleTasks.Remove(fullResultPacket.Header.BattleID);
	}

	public TurnBattle GetRunningBattle(GameId id)
	{
		return ((BattleTaskExecutor) BattleTasks[id].Executor).Battle;
	}
