...
public class ComponentSynchronizer : IComponentSync
{
	...
	private readonly Dictionary<Type, List<Action<IEntity, object>>> _componentAdded = new();
	private readonly Dictionary<Type, List<Action<IEntity, object>>> _componentRemovals = new();
	private readonly Dictionary<Type, List<Action<IEntity, object, object>>> _componentSyncs = new();
	private readonly Dictionary<Type, List<Type>> _listeners = new();

	private readonly List<(object, object)> _toSync = new();
	...
	public void RemoveListener(object listener)
	{
		...
		{
			if (_componentRemovals.TryGetValue(t, out var removals)) removals.RemoveAll(r => r.Target == listener);
			if (_componentSyncs.TryGetValue(t, out var syncs)) syncs.RemoveAll(r => r.Target == listener);
		}
	}
	...
	public void OnComponentRemoved<ComponentType>(Action<IEntity, ComponentType> OnSync)
	{
		...
		if (OnSync.Target == null) throw new Exception("Target is null");
	...
	public void OnComponentModified<ComponentType>(Action<IEntity, ComponentType, ComponentType> OnSync)
	{
		...
		if (OnSync.Target == null)
		{
			syncList.Add((entity, oldComponent, newComponent) =>
				OnSync(entity, (ComponentType) oldComponent, (ComponentType) newComponent));
			return;
		}
