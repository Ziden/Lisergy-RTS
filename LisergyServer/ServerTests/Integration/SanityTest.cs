...
///     This is done as a sdk instead of a client so it's easier to test and debug and ensure its logic is fully functional
///     and can be safely
///     exposed to client so the client is only required to ensure the UI and visual representations are working.
/// </summary>
public class SmokeTest
{
	private TestGameClient _client;
	...
	{
		_client.ClearEvents();
		var party = _client.Game.Entities.GetChildren(_client.Modules.Player.LocalPlayer.EntityId, EntityType.Party)
			.First();
		...
		{
			if (tile == null ||
			    !tile.Components.Get<TileVisibilityComponent>().PlayersViewing.Contains(_playerId)) continue;
	...
	private async Task ValidateMapTiles()
	{
		...
		var tileUpdatesReceived = _client.ReceivedPackets
			.Where(p => p is EntityUpdatePacket e && e.Type == EntityType.Tile).Cast<EntityUpdatePacket>();
		...
		var tileViewsCreated =
			_client.EventsInSdk.Count(e => e is EntityViewRendered c && c.Entity.EntityType == EntityType.Tile);
		...
		foreach (var tileUpdate in tileUpdatesReceived)
		{
			...
			Assert.AreEqual(clientTile.SpecId, tileData.TileId);
			Assert.AreEqual(
				new Location(clientTile.X >> GameWorld.CHUNK_SIZE_BITSHIFT,
					clientTile.Y >> GameWorld.CHUNK_SIZE_BITSHIFT), clientTile.Chunk.Position);
	...
	private async Task ValidateEntities()
	{
		...
		foreach (var serverEntity in _server.Game.Entities.AllEntities)
		{
			...
			else
			{
				...
				Assert.AreEqual(clientEntity.Get<MapPlacementComponent>()?.Position,
					serverEntity.Get<MapPlacementComponent>()?.Position);
			}
		}
	...
	private async Task ValidateIsHarvesting(IEntity party, TileModel resourceTile)
	{
		...
		var entityUpdate = await _client.WaitUntilReceives<EntityUpdatePacket>(p =>
			p.SyncedComponents.Any(c => c.GetType() == typeof(HarvestingComponent)));
		var syncedHarvest =
			(HarvestingComponent) entityUpdate.SyncedComponents.First(c => c.GetType() == typeof(HarvestingComponent));
		...
		var tileUpdates = _client.FilterReceivedPackets<EntityUpdatePacket>()
			.Where(p => p.EntityId == resourceTile.EntityId).ToList();
	...
	private async Task MoveEntityTo(IEntity party, TileModel resourceTile, CourseIntent intent)
	{
		...
		while (party.GetTile() != resourceTile) await _client.WaitUntilReceives<EntityUpdatePacket>();
	...
	private async Task ValidateMapResources()
	{
		...
		foreach (var tile in ((GameWorld) _client.Game.World).AllTiles().Where(t => t != null))
			if ((tile.Components.Has<TileResourceComponent>() && !tile.HasHarvestSpot) ||
			    (!tile.Components.Has<TileResourceComponent>() && tile.HasHarvestSpot))
