...
internal class TestGameClient : LisergySDK, IDisposable, IEventListener
{
	private readonly StandaloneServer _server;

	public TestGameClient(StandaloneServer server)
	{
		_server = server;
		...
		ClientEvents.On<GameStartedEvent>(this, OnGameStart);
	}

	public new ClientNetwork Network { get; }
	public List<BasePacket> ReceivedPackets { get; } = new();
	public List<BasePacket> SentPackets { get; } = new();
	public List<IBaseEvent> EventsInClientLogic { get; } = new();
	public List<IClientEvent> EventsInSdk { get; } = new();
	...
	public List<T> FilterClientEvents<T>()
	{
		...
		foreach (var e in inLogic) all.Add((T) e);
		foreach (var e in inSDK) all.Add((T) e);
		return all;
	}
	...
	public async Task WaitUntilPacketCount<T>(int count, Func<T, bool> validate = null, int timeout = 20)
		where T : BasePacket
	{
		Network.Tick();
	...
	{
		Tick();
		var updates = FilterReceivedPackets<EntityUpdatePacket>().Where(p =>
			p.EntityId == e.EntityId && p.SyncedComponents.Any(c => c.GetType() == typeof(T)));
		var tries = 10;
		while (tries > 0 && updates.Count() == 0)
		{
			tries--;
			Tick();
			updates = FilterReceivedPackets<EntityUpdatePacket>().Where(p =>
				p.EntityId == e.EntityId && p.SyncedComponents.Any(c => c.GetType() == typeof(T)));
			await Task.Delay(100);
		}

		Assert.Greater(updates.Count(), 0,
			$"Did not receive component {typeof(T).Name} update from entity {e.EntityId}");
