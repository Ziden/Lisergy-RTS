...
public class TestGameClientSDKSetup
{
	...
	private PlayerModel _serverPlayer;
	...
	public void TestTilesBeingSent()
	{
		...
		Assert.AreEqual(tilesVisibleOnServer.Count,
			_client.FilterClientEvents<EntityViewRendered>().Where(e => e.Entity.EntityType == EntityType.Tile)
				.Count());
		foreach (var loc in tilesVisibleOnServer)
		{
	...
	public void TestBuildingOnTile()
	{
		foreach (var serverTile in (_serverLogic.World as GameWorld).AllTiles())
		{
			...
			if (serverBuilding == null) continue;
	...
	public void TestEntityExploring()
	{
		...
		foreach (var loc in visibleTiles)
			_client.Network.ReceiveInput(
				_playerId,
				_serverLogic.World.GetTile(loc).Entity.Logic.DeltaCompression.GetUpdatePacket(_playerId, false));
		...
		Assert.AreEqual(clientParty.Get<MapPlacementComponent>().Position,
			serverParty.Get<MapPlacementComponent>().Position);
