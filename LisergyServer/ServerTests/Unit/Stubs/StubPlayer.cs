...
public class TestServerPlayer : PlayerModel, IEventListener
{
	...
	public List<BasePacket> ReceivedPackets = new();
	public List<IBaseEvent> TriggeredEvents = new();
	...
	public IEntity GetParty(int ind)
	{
		return Parties[ind];
	}
	...
	public void SendMoveRequest(IEntity p, TileModel t, CourseIntent intent)
	{
		...
		var ev = new MoveEntityCommand {Path = path.ToArray(), Entity = p.EntityId, Intent = intent};
		ev.Sender = player;
