...
public class TestGame : LisergyGame
{
	...
	public TestGame(GameSpec specs = null, bool createWorld = true, bool createPlayer = true) : base(
		specs ?? GetTestSpecs(), GetTestLog())
	{
		GameId.INCREMENTAL_MODE = 1;
		if (createWorld) CreateTestWorld();
		...
		TestNetwork.OnOutgoingPacket +=
			(player, packet) => (Players[player] as TestServerPlayer)?.SendTestPacket(packet);
		if (createPlayer && createWorld)
			CreatePlayer();
		var originalByteCount = GC.GetTotalMemory(false) / 1000;
	...
	public List<BasePacket> SentServerPackets { get; protected set; } = new();
	public GameWorld TestWorld => World as GameWorld;
	public ServerChunkMap WorldChunks => TestWorld.Chunks;

	public GameScheduler GameScheduler => Scheduler as GameScheduler;
	...
	{
		foreach (var e in game.Entities.AllEntities) e.Components.ValidateComponentSetModifications();
	}
	...
	public void HandleClientEvent<T>(PlayerModel sender, T ev) where T : BasePacket
	{
		...
		if (deserialized is IGameCommand c) c.Execute(sender.Game);
	...
	{
		PlayerModel pl;
		World.Players.GetPlayer(_testPlayerId, out pl);
	...
	{
		if (_testSpecs == null) _testSpecs = TestSpecs.Generate();
	...
	{
		return Specs.Buildings.Values.RandomElement();
	}
	...
	public TileModel RandomNotBuiltTile()
	{
		...
		foreach (var tile in tiles)
			if (tile.Logic.Tile.GetBuildingOnTile() == null)
				return tile;
		throw new Exception("No unbuilt tile");
	}
}