...
namespace UnitTests
{
    public class TestHarvesting
    {
        ...
        public void TestBeginHarvesting()
        {
            ...
            Assert.AreEqual(_game.GameTime, harvestingComponent.StartedAt);
        }
        ...
        public unsafe void TestCourseToHarvest()
        {
            ...
            var ev = new MoveRequestPacket() { Path = new List<Position>() { _logs.Position }, PartyIndex = _party.PartyIndex, Intent = CourseIntent.Harvest };
            ev.Sender = _player;
            ...
            ev = new MoveRequestPacket() { Path = new List<Position>() { _logs.GetNeighbor(Direction.SOUTH).Position }, PartyIndex = _party.PartyIndex };
            ev.Sender = _player;
            _game.HandleClientEvent(_player, ev);
            _game.GameScheduler.Tick(_game.GameTime + _party.Course.Delay);
