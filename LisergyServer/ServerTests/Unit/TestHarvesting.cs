...
public class TestHarvesting
{
	...
	public void TestStartHarvestingSendingUpdatePackets()
	{
		...
		var tileUpdates = _game.SentServerPackets.Where(p =>
			p is EntityUpdatePacket up && up.Type == EntityType.Tile && up.EntityId == _logs.EntityId).ToList();
	...
	[Test]
	public void TestHarvestingComponentSync()
	{
		var tileResourceBefore = _logs.Get<TileResourceComponent>();
		_game.Network.DeltaCompression.ClearDeltas();
		_party.Logic.Harvesting.StartHarvesting(_logs);
		...
		var firstUpdate =
			entityUpdate1.SyncedComponents.FirstOrDefault(c => c.GetType() == typeof(HarvestingComponent));
		var secondUpdate =
			entityUpdate2.SyncedComponents.FirstOrDefault(c => c.GetType() == typeof(HarvestingComponent));
	...
	[Test]
	public void TestHarvestingComponentRemoved()
	{
		var tileResourceBefore = _logs.Get<TileResourceComponent>();
		_game.Network.DeltaCompression.ClearDeltas();
		_party.Logic.Harvesting.StartHarvesting(_logs);
	...
	public void TestFinishHarvestingHalf()
	{
		...
		Assert.AreEqual(cargo.CurrentWeight,
			harvestedStack.Amount * _game.Specs.Resources[_logs.HarvestPointSpec.ResourceId].WeightPerUnit);
	}
	...
	public void TestFinishHarvestingHalfTwice()
	{
		...
		Assert.AreEqual(cargo.CurrentWeight,
			cargo.Items.First().Value * _game.Specs.Resources[_logs.HarvestPointSpec.ResourceId].WeightPerUnit);
	}

	[Test]
	public void TestCourseToHarvest()
	{
		Assert.IsFalse(_party.Components.Has<HarvestingComponent>());

		// Course starts harvesting
		var ev = new MoveEntityCommand
			{Path = new Location[] {_logs.Position}, Entity = _party.EntityId, Intent = CourseIntent.Harvest};
		ev.Sender = _player;
		...
		ev = new MoveEntityCommand
			{Path = new Location[] {_logs.GetNeighbor(Direction.SOUTH).Position}, Entity = _party.EntityId};
		ev.Sender = _player;
		_game.HandleClientEvent(_player, ev);
		_game.GameScheduler.Tick(_game.GameTime + _party.Course().Delay);
