...
public class TestJoinWorld
{
	...
	public void TestJoinInitialDataEventsSent()
	{
		...
		var updateEvents = clientPlayer.ReceivedPacketsOfType<EntityUpdatePacket>()
			.Where(e => e.Type != EntityType.Tile).ToList();
		...
		var partyPosition =
			(MapPlacementComponent) partyEvent.SyncedComponents.FirstOrDefault(c =>
				c.GetType() == typeof(MapPlacementComponent));
		var buildingPosition =
			(MapPlacementComponent) buildingEvent.SyncedComponents.FirstOrDefault(c =>
				c.GetType() == typeof(MapPlacementComponent));
	...
	public void TestNewPlayerReceivingEvents()
	{
		...
		var entityUpdates = player.ReceivedPacketsOfType<EntityUpdatePacket>().Where(e => e.Type != EntityType.Tile)
			.ToList();
