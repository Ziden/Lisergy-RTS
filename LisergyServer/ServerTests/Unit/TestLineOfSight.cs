...
public class TestLineOfSight
{
	...
	public void TestCreatingExploringEntityLOS()
	{
		...
		var entityUpdates = player.ReceivedPacketsOfType<EntityUpdatePacket>().Where(p => p.Type != EntityType.Tile)
			.ToList();
		var tileVisibleEvent = player.ReceivedPacketsOfType<EntityUpdatePacket>().Where(p => p.Type == EntityType.Tile)
			.ToList();
	...
	public void TestKeepingOneEntityExploring()
	{
		...
		*/
		...
		Assert.IsTrue(!tileTopOfBuilding.Logic.Vision.GetEntitiesViewing().Contains(party.EntityId),
			"Party is not seeing the tile");
		Assert.IsTrue(tileTopOfBuilding.Logic.Vision.GetEntitiesViewing().Contains(building.EntityId),
			"Building still seeing the tile");
	}
	...
	public void TestEventsAndPacketsSync()
	{
		...
		var tileUpdates = player.ReceivedPacketsOfType<EntityUpdatePacket>().Where(p => p.Type == EntityType.Tile)
			.ToList();
