namespace GameUnitTests
{
    public class TestPersistence
    {
        ...
        public void TestSavingWorld()
        {
            ...
            var mapSize = (ushort)game1.World.TilemapDimensions.x;
        ...
        private void CompareGames(LisergyGame game1, LisergyGame game2)
        {
            ...
            foreach (var p1 in game1.World.Players.AllPlayers())
            {
                ...
                Assert.IsTrue(p1.Components.GetReference<PlayerData>().StoredUnits.Values.SequenceEqual(p2.Components.GetReference<PlayerData>().StoredUnits.Values));
                Assert.IsTrue(p1.Components.GetReference<VisibilityData>().OnceExplored.SequenceEqual(p2.Components.GetReference<VisibilityData>().OnceExplored));
                Assert.IsTrue(p1.Components.GetReference<VisibilityData>().VisibleTiles.SequenceEqual(p2.Components.GetReference<VisibilityData>().VisibleTiles));
                foreach (var visible in p1.Components.GetReference<VisibilityData>().VisibleTiles)
                {
                    var tile = game2.World.GetTile(visible.X, visible.Y);
            ...
            foreach (var e1 in ((GameEntities)game1.Entities).AllEntities)
            {
                ...
                Assert.AreEqual(e1.GetTile().Position, e2.GetTile().Position);
