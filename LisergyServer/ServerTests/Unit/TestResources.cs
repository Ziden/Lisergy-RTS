	...
	public void TestTileResourcesInitialSetup()
	{
		...
		foreach (var tilePosition in clientPlayer.VisibilityData.VisibleTiles)
		{
			...
			if (tile.Components.Has<TileResourceComponent>())
			{
				...
				var packet = clientPlayer.ReceivedPackets.First(p =>
					p is EntityUpdatePacket up && up.Type == EntityType.Tile &&
					up.GetComponent<TileDataComponent>().Position == tile.Position) as EntityUpdatePacket;
