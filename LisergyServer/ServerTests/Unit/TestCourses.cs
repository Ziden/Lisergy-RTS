...
public class TestMovement
{
	...
	{
		var ev = new MoveEntityCommand {Path = _path.ToArray(), Entity = _party.EntityId};
		ev.Sender = _player;
	...
	public void TestMoveOutsidePlayerVision()
	{
		...
		_game.HandleClientEvent(player2, new MoveEntityCommand {Path = _path.ToArray(), Entity = player2Party.EntityId});
		...
		var moveEvents = _player.ReceivedPacketsOfType<EntityUpdatePacket>().Where(p =>
			p.EntityId == player2Party.EntityId && p.SyncedComponents.Any(c => c is MapPlacementComponent));
	...
	public void TestMoveEvents()
	{
		...
		var moveEvents = _player.ReceivedPacketsOfType<EntityUpdatePacket>().Where(p =>
			p.EntityId == _party.EntityId && p.SyncedComponents.Any(c => c is MapPlacementComponent));
