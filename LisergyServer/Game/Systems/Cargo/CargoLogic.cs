...
namespace Game.Systems.Resources
{
    ...
    /// </summary>
    public class CargoLogic : BaseEntityLogic<CargoComponent>
	{
        /// <summary>
		...
		{
			var cargo = CurrentEntity.Get<CargoComponent>();
			if (cargo.Items.TryGetValue(id, out var amount)) return amount;
			return 0;
		}
		...
		{
			var cargo = CurrentEntity.Get<CargoComponent>();
			if (cargo.GetAmount(resource) < amt) return false;
        ...
        public bool AddTocargo(in ResourceStackData resource)
		{
			...
			{
				Game.Log.Error(
					$"Cargo capacity rached for {resource} to cargo {CurrentEntity.Get<CargoComponent>()} from {CurrentEntity}");
				return false;
			}
			...
			if (cargo.Items == null) cargo.Items = new Dictionary<ResourceSpecId, ushort>();
