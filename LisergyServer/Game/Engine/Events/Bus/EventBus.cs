...
namespace Game.Engine.Events.Bus
{
	...
	{
		private readonly Dictionary<Type, List<ListenerWrapper>> _listeners =
			new Dictionary<Type, List<ListenerWrapper>>();
		...
		public void On<EvType>(IEventListener listener, Action<EvType> callback)
		{
			...
			if (!_listeners.ContainsKey(eventType)) _listeners[eventType] = new List<ListenerWrapper>();
			_listeners[eventType].Add(new ListenerWrapper(listener, (Action<object>) (ev => callback((EvType) ev))));
		}

		public void RemoveListener(IEventListener listener)
		{
			foreach (var key in _listeners.Keys) _listeners[key].RemoveAll(l => l.Listener == listener);
		}
	}
}