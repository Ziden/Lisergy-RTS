...
namespace Game.Engine.Network
{
	public class DeltaCompression
	{
        ...
        /// </summary>
        private static readonly HashSet<GameId> _stoppedSeeingEntity = new HashSet<GameId>();
		...
		{
			if (!Enabled) return;
			foreach (var tracked in _modifiedEntities) tracked.Logic.DeltaCompression.Clear();
			foreach (var tracked in _modifiedTiles) tracked.Logic.DeltaCompression.Clear();
        ...
        private void SendUpdateToNewViewers(IEntity entity)
		{
			...
			{
				if (hasPreviousTile) entity.Components.Remove<PreviousMapPlacementComponent>(); // TODO WTF ?
