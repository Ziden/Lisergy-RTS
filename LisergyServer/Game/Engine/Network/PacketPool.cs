...
namespace Game.Engine.Network
{
    ...
    public static class PacketPool
    {
        ...
        public static IReadOnlyDictionary<Type, Queue<BasePacket>> GetFree() => _free;
        public static IReadOnlyDictionary<Type, HashSet<BasePacket>> GetUsed() => _used;
        ...
        {
            if(_used.TryGetValue(item.GetType(), out var used))
            {
                used.Remove(item);
            }
