...
namespace Game.World
{
	...
	public class GameWorld : IGameWorld
	{
		...
		public List<ChunkPopulator> ChunkPopulators { get; } = new List<ChunkPopulator>();
		...
		public TileModel GetUnusedStartingTile()
		{
			...
			if (freeChunk == null) throw new Exception("No more room for newbie players in this world");
		...
		{
			return Chunks.ValidCoords(tileX, tileY);
		}

		public Chunk GetChunk(in int chunkX, in int chunkY)
		{
			return Chunks.GetChunk(chunkX, chunkY);
		}

		public Location[] FindPath(TileModel from, TileModel to)
		{
			return Chunks.FindPath(from, to);
		}

		public void CreateMap(in ushort sizeX, in ushort sizeY)
		{
			Chunks.CreateMap(sizeX, sizeY);
		}

		public TileModel GetTile(in int tileX, in int tileY)
		{
			return Chunks.GetTile(tileX, tileY);
		}

		public TileModel GetTile(in Location p)
		{
			return Chunks.GetTile(p);
		}

		public Chunk GetTileChunk(in Location p)
		{
			return Chunks.GetTileChunk(p);
		}
