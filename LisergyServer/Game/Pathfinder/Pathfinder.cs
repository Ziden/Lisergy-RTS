...
namespace Game.Pathfinder
{
    ...
    {
        private readonly CachedChunkMap mGrid = null;
        ...
        private readonly HeuristicFormula mFormula = HeuristicFormula.Manhattan;
        private readonly bool mDiagonals = false;
        private readonly int mHEstimate = 2;
        private readonly bool mPunishChangeDirection = false;
        private readonly bool mTieBreaker = false;
        private readonly bool mHeavyDiagonals = false;
        private readonly int mSearchLimit = 300;
        ...
        public List<PathFinderNode> FindPath(Position start, Position end)
        {
            ...
            while (mOpen.Count > 0 && !mStop)
            {
                ...
                for (int i = 0; i < (mDiagonals ? 8 : 4); i++)
                {
                    ...
                    int newG = mHeavyDiagonals && i > 3
                        ? parentNode.G + (int)(mGrid[newNode.X, newNode.Y].MovementFactor * 2.41)
                        : parentNode.G + (mGrid[newNode.X, newNode.Y].Passable ? 1 : 0);
                    ...
                    //if (foundInOpenIndex == -1)
                    _ = mOpen.Push(newNode);
                }
                mClose.Add(parentNode);
            }
            if (found)
            {
                PathFinderNode fNode = mClose[^1];
