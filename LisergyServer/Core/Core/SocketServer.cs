...
public abstract class SocketServer
{
	...
	private readonly ConsoleCommandExecutor _commandExecutor;
	...
	{
		if (!_socketServer.Start(_port)) throw new Exception("Error listening " + _port);
		while (!_socketServer.Active) await Task.Delay(100);
		...
		while (_socketServer.GetNextMessage(out _pooledMessage))
			switch (_pooledMessage.eventType)
			{
				...
				case EventType.Data:
					...
					Log.Debug(
						$"Received {_pooledMessage.data.Length} bytes for {_packet} from connection {_pooledMessage.connectionId}");
