...
public class FlatFileWorldPersistence : IWorldPersistance
{
	...
	public byte[] SerializeEntities(LisergyGame game)
	{
		return null;
		/*
	...
	public void DeserializeEntities(LisergyGame game, byte[] entities)
	{
		...
		foreach (var e in deserialized)
		{
			...
			foreach (var component in e.Components) created.Components.Save(component);
	...
	{
		return
			null; // Serialization.FromAnyTypes(game.World.Players.AllPlayers().Select(p => new SerializedPlayer(p)).ToList()).ToArray();
	}
	...
	public void DeserializeMap(LisergyGame game, byte[] data)
	{
		/*
	...
	public byte[] SerializeMap(LisergyGame game)
	{
		//var map = (ServerChunkMap)game.World.Map;
