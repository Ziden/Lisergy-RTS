...
{
	public class EntityVisionLogic : BaseEntityLogic<EntityVisionComponent>
	{
		private readonly HashSet<TileModel> _newLineOfSight = new HashSet<TileModel>();

        /// <summary>
        ///     Line of sight hashsets cached so no need to keep instantiating
        /// </summary>
        private readonly HashSet<TileModel> _oldLineOfSight = new HashSet<TileModel>();
        ...
        public void UpdateVisionRange(TileModel from, TileModel to)
		{
			...
			if (los > 0)
			{
				...
				foreach (var newTileSeen in newExplored) OnTileExplorationChanged(newTileSeen, explorer, true);

				var unexplored = _oldLineOfSight.Except(_newLineOfSight);
				foreach (var oldTileUnseen in unexplored) OnTileExplorationChanged(oldTileUnseen, explorer, false);
			}
		}
