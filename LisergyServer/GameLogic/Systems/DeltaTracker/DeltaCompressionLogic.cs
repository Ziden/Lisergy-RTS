...
{
	public class DeltaCompressionLogic : BaseEntityLogic<DeltaFlagsComponent>
	{
        /// <summary>
		...
		{
			if (!CurrentEntity.Components.TryGet<PlayerVisibilityComponent>(out var visibilityData))
				throw new Exception("Requires VisibilityData Component");
		...
		public void SetTileExplorationFlag(DeltaFlag flag)
		{
			...
			foreach (var e in onTile) e.Logic.DeltaCompression.SetFlag(flag);
		...
		public bool SetFlag(DeltaFlag flag)
		{
			...
			var hasUpdated = deltas.SetFlag(flag);
			CurrentEntity.Save(deltas);
			if (hasUpdated) Game.Network.DeltaCompression.AddModified(CurrentEntity);
			return hasUpdated;
		}
