...
namespace Game.Systems.Resources
{
    ...
    /// </summary>
    public class HarvestingLogic : BaseEntityLogic<HarvesterComponent>
	{
        /// <summary>
		...
		{
			return CurrentEntity.Components.Has<HarvestingComponent>();
		}
        ...
        public (TimeBlock time, ResourceStackData resource)? GetPossibleHarvest(TileModel tile)
		{
			...
			if (!tileSpec.ResourceSpotSpecId.HasValue) return default;
        ...
        public ResourceStackData StopHarvesting()
		{
			...
			var finalStack =
				new ResourceStackData(harvestState.Resources.Resource.ResourceId, harvestState.AmountHarvested);
			...
			Game.Log.Debug(
				$"{CurrentEntity} finished harvesting on {tile} and got {harvestState.AmountHarvested}x{spec.Name}");
			return finalStack;
		}
	}
}